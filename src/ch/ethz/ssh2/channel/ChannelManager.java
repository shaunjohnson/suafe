 ...
 * @version $Id: ChannelManager.java,v 1.11 2005/12/07 10:25:48 cplattne Exp $
 */
public class ChannelManager implements MessageHandler
{
	...
	public void unRegisterX11Cookie(String hexFakeCookie, boolean killChannels)
	{
		...
		for (int i = 0; i < channel_copy.size(); i++)
		{
			...
			try
			{
				closeChannel(c, "Closing X11 channel since the corresponding session is closing");
			}
			catch (IOException e)
			{
			}
	...
	public void closeAllChannels()
	{
		...
		for (int i = 0; i < channel_copy.size(); i++)
		{
			...
			try
			{
				closeChannel(c, "Closing all channels");
			}
			catch (IOException e)
			{
			}
	...
	public void msgChannelExtendedData(byte[] msg, int msglen) throws IOException
	{
		...
		{
			if (c.state == Channel.STATE_CLOSED)
				return; // ignore (we may have just send a close message)
	...
	public void msgGlobalRequest(byte[] msg, int msglen) throws IOException
	{
		...
		if (log.isEnabled())
			log.log(80, "Got SSH_MSG_GLOBAL_REQUEST");
	}
	...
	public void msgGlobalFailure(byte[] msg, int msglen) throws IOException
	{
		synchronized (channels)
		{
			globalFailedCounter++;
