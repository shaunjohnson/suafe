 ...
 * @version $Id: RemoteX11AcceptThread.java,v 1.3 2005/12/07 10:25:48 cplattne Exp $
 */
public class RemoteX11AcceptThread extends Thread
{
	...
	public void run()
	{
		try
		{
			 ...
			 * Scheifler/Gettys,
			 * X Windows System: Core and Extension Protocols: X Version 11, Releases 6 and 6.1
			 * ISBN 1-55558-148-X (from the ETH library =)
			 */
			...
			c.cm.closeChannel(c, "EOF on both X11 streams reached.");
			s.close();
		}
		catch (IOException e)
		{
			...
			try
			{
				c.cm.closeChannel(c, "IOException in X11 proxy code (" + e.getMessage() + ")");
			}
