...
public class Application {
	 ...
	 * @throws ApplicationException
	 */
	private static void summaryReport(PrintStream out) throws ApplicationException {
		out.println("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Strict//EN\"");
		out.println("\"http://www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd\">");
		out.println("<html xmlns=\"http://www.w3.org/1999/xhtml\" lang=\"en\" xml:lang=\"en\">");
		out.println("<head>");
		out.println("<title>Summary Report</title>");
		out.println("<meta http-equiv=\"Content-Type\" content=\"text/html;charset=utf-8\" />");
		out.println("</head><body>");
		out.println("<h1>Summary Report</h1>");
		out.println("<p>[" + createLink("repositories", "Repositories") + "] " +
		...
		out.println("<h2>" + createAnchor("repositories", "Repositories") + "</h2>");
		out.print("<blockquote>");
		
		if (repositories.size() == 0) {
			out.println("<p>No repositories.</p>");
		}
		else {
			out.println("<ul>");
			
			for (Repository repository : repositories) {
				out.println("<li>" + createReposLink(repository.getName()) + "</li>");
			}
		...
		out.println("<h2>Server Level Access Rules</h2>");
		...
		if (serverPaths.size() > 0) {
			...
			for (Path path : serverPaths) {
				...
				out.println("<p><strong>" + path.getPath() + "</strong></p><ul>");
				
				for (AccessRule rule : rules) {
					if (rule.getGroup() != null) {
						out.println("<li>" + createGroupLink(rule.getGroup().getName()) + " = " + rule.getLevelFullName() + "</li>");
					}
					else if (rule.getUser() != null) {
						out.println("<li>" + createUserLink(rule.getUser().getName()) + " = " + rule.getLevelFullName() + "</li>");
					}
					else {
		...
		if (repositories.size() > 0) {
			for (Repository repository : repositories) {
				out.println("<h2>" + createReposAnchor(repository.getName()) + "</h2>");
		...
		out.println("<h2>" + createAnchor("groups", "Groups") + "</h2>");
		out.print("<blockquote>");
		
		for (Group group : groups) {
			out.println("<p><strong>" + createGroupAnchor(group.getName()) + "</strong></p>");
			...
			out.println("<blockquote><p>Members</p>");
			
			if (groupMembers.size() == 0 && userMembers.size() == 0) {
				out.println("<blockquote><p>No members</p></blockquote>");
			}
			else {
				out.println("<ul>");
				
				for (Group groupMember : groupMembers) {
					out.println("<li>" + createGroupLink(groupMember.getName()) + "</li>");
				}
				
				for (User userMember : userMembers) {
					out.println("<li>" + createUserLink(userMember.getName()) + "</li>");
				}
				
				out.println("</ul>");
			}
						
			out.println("<p>Access Rules</p>");
			
			if (rules.size() == 0) {
				out.println("<blockquote><p>No access rules</p></blockquote>");
			}
			else {
				out.println("<ul>");
				...
				for (AccessRule rule : rules) {
					...
					out.println("<li>[" + repositoryName + path + "] = " + rule.getLevelFullName() + "</li>");
				}
		...
		out.println("<h2>" + createAnchor("users", "Users") + "</h2>");
		out.print("<blockquote>");
		
		for (User user : users) {
			out.println("<p><strong>" + createUserAnchor(user.getName()) + "</strong></p>");
			...
			out.println("<blockquote><p>Groups</p>");
			
			if (userGroups.size() == 0) {
				out.println("<blockquote><p>No groups</p></blockquote>");
			}
			else {
				out.println("<ul>");
				
				for (Group userGroup : userGroups) {
					out.println("<li>" + createGroupLink(userGroup.getName()) + "</li>");
				}
		...
		out.println("<h2>" + createAnchor("projects", "Projects") + "</h2>");
		...
		if (repositories.size() > 0) {
			for (Repository repository : repositories) {
				...
				out.println("<h3>" + createReposAnchor(repository.getName()) + "</h3>");
				
				out.print("<blockquote>");
				
				if (projects.size() == 0) {
					out.println("<p>No projects</p>");
				}
				
				for (String key : projects.keySet()) {
					Project projectBean = projects.get(key);
					
					out.println("<p><strong>" + key + "</strong></p><blockquote>");
					
					out.println("<p>Branches</p>");
		...
		out.println("<hr/>");
		out.println("<p>Generated " + df.format(new Date()) + " using <a href=\"" +
				ResourceUtil.getString("application.url") + "\">" +
				ResourceUtil.getString("application.nameversion") + "</a><br/>");
		out.println("Valid XHTML 1.0 Strict</p>");
